...
def logBasicSettings(log_file_path, input_basins, interval):
    with open (log_file_path, 'a+') as f:
        f.write('\n######################################################################\n')
        f.write('Executing Tool: Ridge Layout and Profile\n')
...
output_lines_name = 'Ridge_Lines'
output_lines_path = path.join(wascob_fd, output_lines_name)
output_stations_name = 'Ridge_Station_Points'
...
try:
    ...
    TableToTable(output_stations_path, tables_dir, 'Ridge_Stations.dbf')
    ...
    AddMsgAndPrint('\nRidge Layout and Profile completed successfully', log_file_path=log_file_path)
    ...
    try:
        AddMsgAndPrint(errorMsg('Ridge Layout and Profile'), 2, log_file_path)
    except:
