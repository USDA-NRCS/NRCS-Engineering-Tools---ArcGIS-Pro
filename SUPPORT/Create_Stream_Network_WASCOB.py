...
def logBasicSettings(log_file_path, wascob_dem, input_culverts, stream_threshold):
    with open (log_file_path, 'a+') as f:
        f.write('\n######################################################################\n')
        f.write('Executing Tool: Create Stream Network (WASCOB)\n')
        f.write(f"Pro Version: {GetInstallInfo()['Version']}\n")
        f.write(f"User Name: {getuser()}\n")
        f.write(f"Date Executed: {ctime()}\n")
        f.write('User Parameters:\n')
        f.write(f"\tProject WASCOB DEM: {wascob_dem}\n")
...
### Input Parameters ###
wascob_dem = GetParameterAsText(0)
input_culverts = GetParameterAsText(1)
stream_threshold = float(GetParameterAsText(2))

### Locate Project GDB ###
wascob_dem_path = Describe(wascob_dem).CatalogPath
if '_WASCOB.gdb' in wascob_dem_path and 'DEM' in wascob_dem_path:
    wascob_gdb = wascob_dem_path[:wascob_dem_path.find('.gdb')+4]
else:
    AddMsgAndPrint('\nThe selected WASCOB DEM is not from an Engineering Tools project or is not compatible with this version of the toolbox. Exiting...', 2)
    exit()
...
streams_path = path.join(wascob_gdb, 'Layers', streams_name)
flow_accum_name = 'Flow_Accumulation_WASCOB'
flow_accum_path = path.join(wascob_gdb, flow_accum_name)
flow_dir_name = 'Flow_Direction_WASCOB'
flow_dir_path = path.join(wascob_gdb, flow_dir_name)
...
try:
    ...
    logBasicSettings(log_file_path, wascob_dem_path, input_culverts, stream_threshold)
    ...
    if input_culverts:
        ...
        if int(GetCount(culverts_path).getOutput(0)) > 0:
            ...
            culverts_min_value = ZonalStatistics(culverts_buffer_temp, 'ZONE', wascob_dem_path, 'MINIMUM', 'NODATA')

            # Elevation cells that overlap the culverts will get the minimum elevation value
            mosaic_list = f"{wascob_dem_path};{culverts_min_value}"
    ...
    for lyr in map.listLayers():
        if lyr.supports("NAME"):
            if '02. Create Stream Network' in lyr.name:
                map.removeLayer(lyr)
    ...
    AddMsgAndPrint('\nCreate Stream Network (WASCOB) completed successfully', log_file_path=log_file_path)
    ...
    try:
        AddMsgAndPrint(errorMsg('Create Stream Network (WASCOB)'), 2, log_file_path)
    except:
        AddMsgAndPrint(errorMsg('Create Stream Network (WASCOB)'), 2)
