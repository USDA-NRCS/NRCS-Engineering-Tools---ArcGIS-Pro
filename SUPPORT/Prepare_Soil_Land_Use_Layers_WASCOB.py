...
output_soils_name = f"{basins_name}_Soils_WASCOB"
output_soils_path = path.join(wascob_fd, output_soils_name)
output_landuse_name = f"{basins_name}_Land_Use_WASCOB"
...
if not len(ListFields(input_basins, 'Subbasin')) > 0:
    AddMsgAndPrint('\nSubbasin field was not found in input Basins layer. Exiting...', 2)
    exit()
...
try:
    ...
    if input_boundaries:
        Dissolve(basins_path, basins_dissolve_temp, '', '', 'MULTI_PART', 'DISSOLVE_LINES')
        Clip(input_boundaries, basins_path, boundaries_clip_temp)
        Union([boundaries_clip_temp, basins_dissolve_temp], land_use_temp, 'NO_FID')
        MultipartToSinglepart(land_use_temp, output_landuse_path)
        AddMsgAndPrint('\nCreated Land Use layer from dissolved basins and input boundaries intersection...', log_file_path=log_file_path)
    else:
        Dissolve(basins_path, output_landuse_path, '', '', 'MULTI_PART', 'DISSOLVE_LINES')
        AddMsgAndPrint('\nCreated Land Use layer from dissolved basins...', log_file_path=log_file_path)
    ...
    domains = Describe(wascob_gdb).domains
    if not 'LandUse_Domain' in domains:
        TableToDomain(tr_55_land_use_table, 'LANDUSE', 'LANDUSE', wascob_gdb, 'LandUse_Domain', 'LandUse_Domain', 'REPLACE')
    ...
    if not 'Hydro_Domain' in domains:
        TableToDomain(hydro_groups_table, 'HydrolGRP', 'HydrolGRP', wascob_gdb, 'Hydro_Domain', 'Hydro_Domain', 'REPLACE')
    ...
    AddMsgAndPrint(f"\tNOTICE: Before calculating the Runoff Curve Number for the basin(s), {output_landuse_name} layer requires attribution for LANDUSE fields, and any combined, invalid, or NULL Hydrologic Group values in {output_soils_name} must be addressed.", 1, log_file_path)
